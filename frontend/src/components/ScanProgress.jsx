...
export default function ScanProgress({ scanConfig, scanResult, onComplete }) {
  ...
  const attempts = scanResult?.threat_model?.attempts || scanResult?.attack_analysis?.attempts || [];
  const evaluation = scanResult?.evaluation || {};
  const isVeto = evaluation.status === 'CRITICAL_VETO' || scanResult?.metadata?.verdict === 'VETO' || scanResult?.verdict === 'VETO';
