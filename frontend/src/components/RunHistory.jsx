          ...
          <table className="w-full text-left font-mono text-xs">
            ...
            <tbody className="divide-y divide-[#22222a] bg-[#0d0e12]">
              {runs.map((r) => {
                ...
                const runNumber = r.metadata?.run_number || r.run_number || '#AV-LIVE';
                const agentName = r.metadata?.agent_name || r.agent_name || 'Agent';
                const name = r.metadata?.name || r.name || runId;
                const threat = r.threat_category || r.metadata?.threat_category || 'OWASP ASI';
                ...
                return (
                  ...
                  >
                    <td className="p-4 pl-6 font-medium text-[#70dcd3]">{runNumber}</td>
                    <td className="p-4 font-medium text-white group-hover:text-[#70dcd3] transition-colors">
                      {agentName}
                      <span className="block text-[10px] text-[#a2a4a9] font-normal">{name}</span>
                    </td>
                    <td className="p-4">
                      <span className={clsx(
                    ...
                    <td className="p-4 text-[#aeaeb7]">{r.duration || '< 1s'}</td>
                    <td className="p-4 text-[#60606c]">{r.timestamp || 'Live'}</td>
