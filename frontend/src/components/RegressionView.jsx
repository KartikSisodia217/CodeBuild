...
export default function RegressionView({ data }) {
  ...
  const yamlContent = data?.yaml_content || data?.metadata?.yaml_content;
  ...
  const isVeto = evaluation.status === 'CRITICAL_VETO' || data.metadata?.verdict === 'VETO' || data.verdict === 'VETO';
  ...
  const handleDownload = () => {
    ...
    element.download = `${data.scenario_id || data.run_id || 'regression'}_spec.yaml`;
