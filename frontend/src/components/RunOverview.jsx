...
export default function RunOverview({ data, onSwitchTab }) {
  ...
  const verdict = meta.verdict || data.verdict || data.status || evaluation.status;
  const isVeto = verdict === 'CRITICAL_VETO' || verdict === 'VETO';
  const isPass = verdict === 'PASS';

  const injectionSource = evaluation.injection_source_span_id || data.threat_model?.injection_point || data.attack_analysis?.injection_point || 'None observed';
  const highRiskSink = evaluation.violating_tool || (evaluation.details?.high_risk_sink_reached ? (data.threat_model?.high_risk_sink || data.attack_analysis?.high_risk_sink) : 'None reached');

  const isUnsupported = verdict === 'UNSUPPORTED' || evaluation.status === 'UNSUPPORTED';
  const isNotAgentic = verdict === 'NOT_AGENTIC' || evaluation.status === 'NOT_AGENTIC';
  const isNotScanned = isUnsupported || isNotAgentic || verdict === 'EXECUTION_UNAVAILABLE' || verdict === 'SCAN NOT RUN';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        <div className="p-4 bg-[#0d0e12] border border-[#d9dae5]/16 rounded-[20px]">
          ...
          <div className="text-sm font-medium text-white truncate mt-1">{meta.agent_name || data.trajectory?.agent_name || data.trace?.agent_name}</div>
        </div>

        <div className="p-4 bg-[#0d0e12] border border-[#d9dae5]/16 rounded-[20px]">
          <div className="text-[10px] font-mono text-[#a2a4a9] uppercase tracking-[0.094em]">Run ID</div>
          <div className="text-sm font-medium text-[#70dcd3] font-mono truncate mt-1">{meta.run_number || data.scenario_id || data.run_id}</div>
        </div>
        ...
        <div className="p-4 bg-[#0d0e12] border border-[#d9dae5]/16 rounded-[20px]">
          ...
          <div className="text-sm font-medium text-white font-mono mt-1">{meta.attack_attempts ?? data.threat_model?.attempts?.length ?? data.attack_analysis?.attempts?.length ?? '—'}</div>
        </div>

        <div className="p-4 bg-[#0d0e12] border border-[#d9dae5]/16 rounded-[20px]">
          <div className="text-[10px] font-mono text-[#a2a4a9] uppercase tracking-[0.094em]">Tool Calls</div>
          <div className="text-sm font-medium text-white font-mono mt-1">{data.trajectory?.spans?.length ?? data.trace?.spans?.length ?? meta.tool_calls ?? '—'}</div>
        </div>
