...
function App() {
  ...
  useEffect(() => {
    if (activeChat) {
      document.title = `${activeChat.title} - Legal.ai`;
    } else {
      document.title = "New Chat - Legal.ai";
    }
  }, [activeChat?.title]);
      ...
      {isDragging && (
        <div className="fixed inset-0 bg-[#fdfcf6]/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center border-4 border-dashed border-amber-200 m-4 rounded-xl pointer-events-none animate-fade-in">
          <div className="flex flex-col items-center animate-scale-in">
            ...
            <h2 className="text-3xl font-spectral font-medium text-gray-800">Drop your files to attach to this legal query</h2>
          </div>
        </div>
      )}
    </div>
